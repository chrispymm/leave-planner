...
class SchoolHolidayTest < ActiveSupport::TestCase
  test "validates end date is after or equal to start date" do
    valid = SchoolHoliday.new(title: "Summer", account: accounts(:pymm_account), start_date: Date.new(2026, 7, 20), end_date: Date.new(2026, 9, 1))
    assert valid.valid?

    invalid = SchoolHoliday.new(title: "Invalid", account: accounts(:pymm_account), start_date: Date.new(2026, 7, 20), end_date: Date.new(2026, 7, 19))
  ...
  test "map_by_date returns correct holiday mapping" do
    ...
    sh = SchoolHoliday.create!(title: "Half Term", account: accounts(:pymm_account), start_date: Date.new(2026, 10, 26), end_date: Date.new(2026, 10, 30))
